...
class RemoteDesktop:
    ...
    def stop_server(self):
        if self.active:
            self.active = False
            # Close a dummy connection to break the accept loop
    ...
    def showcords(self, event, x, y, flags, param):
        conn = param
    ...
    def __client_connection(self, connection, address):
        ...
        try:
            ...
            print(f"Error receiving display data: {e}")
        ...
        if self.monitor > 0:
            cv2.createTrackbar("Monitor", str(address), 0, self.monitor - 1, lambda y: y)
        else:
            print(f"[!] Invalid monitor count received: {self.monitor}")
        ...
        while self.active:
            ...
            if frame_data[0:7].decode() == "video->":
                ...
                self.monitor = cv2.getTrackbarPos("Monitor", str(address)) + 1 if self.monitor > 0 else 0
...
if __name__ == '__main__':
    server = RemoteDesktop('localhost', 6969)
    server.start_server()
